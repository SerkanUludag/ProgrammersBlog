...
namespace ProgrammersBlog.Web.Attributes
{
    ...
    public class ViewCountFilterAttribute : Attribute, IAsyncActionFilter
    {
        ...
        {
            var articleId = context.ActionArguments["articleId"]; // get articleId action parameter
            if(articleId is not null)
            {
                string articleValue = context.HttpContext.Request.Cookies[$"article{articleId}"]; // ie: article22
                if (string.IsNullOrEmpty(articleValue))
                {
                    ...
                    await next();   // continue executing function
                }
            }
            await next();
        }
